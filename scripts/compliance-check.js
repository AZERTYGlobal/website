...
// Load files
const masterJSON = JSON.parse(fs.readFileSync(path.join(SITE_ROOT, 'data', MASTER_FILENAME), 'utf8'));
const testerJSON = JSON.parse(fs.readFileSync(path.join(SITE_ROOT, 'tester', 'azerty-global.json'), 'utf8'));
const charIndex = JSON.parse(fs.readFileSync(path.join(SITE_ROOT, 'tester', 'character-index.json'), 'utf8'));
...
console.log('═══════════════════════════════════════════════════════════════════════════════');
console.log(`1. KEYMAP COMPARISON (${MASTER_FILENAME} vs tester/azerty-global.json)`);
console.log('───────────────────────────────────────────────────────────────────────────────\n');
...
for (const keyId of Object.keys(testerKeymap)) {
  if (!masterKeymap[keyId]) {
    if (IGNORED_TESTER_ONLY_KEYS.has(keyId)) continue;
    issues.push({
      type: 'KEYMAP',
      severity: 'LOW',
