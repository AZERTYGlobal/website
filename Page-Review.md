# Page Review - Site AZERTY Global

Pages à relire avant publication après modifications visibles.

Note 11ty : cette checklist s'applique aussi au rendu `dist` quand une source
`src/pages/*.njk` change. Ne pas cocher une page si son rendu 11ty n'a pas ete
revu visuellement. `dist` reste un artefact de build et ne se coche pas
separement.

- [x] `index.html` - Accueil (revu 2026-07-07 : DA spécimen + badge, captures Playwright 360/390/768/1366, 0 overflow)
- [x] `download.html` - Téléchargement (revu 2026-07-07 : DA spécimen + doctrine CTA, captures Playwright 360/390/768/1366)
- [ ] `francais-etranger.html` - Français à l’étranger
- [ ] `guide.html` - Guide
- [ ] `faq.html` - FAQ
- [ ] `aide-memoire.html` - Aide-mémoire
- [ ] `dev.html` - Développeurs
- [ ] `entreprises.html` - Entreprises
- [ ] `ecoles.html` - Écoles
- [ ] `pilote.html` - Pilote gratuit
- [ ] `pilote-bilan.html` - Bilan pilote
- [ ] `a-propos.html` - À propos
- [ ] `comparatif.html` - Comparatif
- [ ] `compatibilite.html` - Compatibilité
- [ ] `presse.html` - Presse
- [ ] `soutien.html` - Soutien
- [ ] `nouveautes.html` - Nouveautés
- [ ] `feedback.html` - Feedback
- [ ] `bug.html` - Bug
- [ ] `licence.html` - Licence
- [ ] `mentions-legales.html` - Mentions légales
- [ ] `merci.html` - Merci
- [ ] `404.html` - Erreur 404
- [ ] `afrique.html` - Afrique
- [ ] `questionnaire.html` - Questionnaire
- [ ] `arobase.html` - Arobase
- [ ] `accolades.html` - Accolades
- [ ] `crochets.html` - Crochets
- [ ] `tiret-cadratin.html` - Tirets typographiques
- [ ] `e-aigu-majuscule.html` - É majuscule
- [ ] `e-grave-majuscule.html` - È majuscule
- [ ] `a-grave-majuscule.html` - À majuscule
- [ ] `c-cedille-majuscule.html` - Ç majuscule
- [ ] `e-dans-l-a.html` - E dans l’a
- [ ] `e-dans-l-o.html` - E dans l’o
- [ ] `guillemets.html` - Guillemets

*Dernière mise à jour : 2026-06-29*
